...
1. **Install dependencies** – Node.js 18 LTS is recommended. The setup script
   attempts to switch via `nvm` if available. Newer Node versions can work but
   may compile TensorFlow from source. Run `npm install` to fetch the
   Human library,
