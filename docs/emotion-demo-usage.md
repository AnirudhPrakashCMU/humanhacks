...
- **Node.js 18 LTS recommended.** `make setup` will try to switch using `nvm`
  if it is available. Newer Node versions can work but may need native build
  tools (e.g. Xcode Command Line Tools on macOS).
...
- **Installation fails with a different Node version**: run `nvm use 18` and reinstall dependencies.
